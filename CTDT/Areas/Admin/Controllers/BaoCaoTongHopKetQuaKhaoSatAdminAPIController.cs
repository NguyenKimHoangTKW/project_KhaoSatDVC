...
namespace CTDT.Areas.Admin.Controllers
{
    ...
    public class BaoCaoTongHopKetQuaKhaoSatAdminAPIController : ApiController
    {
        dbSurveyEntities db = new dbSurveyEntities();
        public class option_bao_cao_tong_hop_admin
        {
        ...
        public async Task<IHttpActionResult> load_pks_by_year([FromBody] option_bao_cao_tong_hop_admin option)
        {
            var pks = await db.survey
                .Where(x => x.id_namhoc == option.namhoc && x.id_hedaotao == option.hedaotao)
                .Select(x => new
                {
                    id_phieu = x.surveyID,
        ...
        public async Task<List<dynamic>> bao_cao_tong_hop_no_dtg(IEnumerable<survey> sortedSurveys, int id_nam_hoc, dynamic response, string check_role, GiamSatThongKeKetQua giamsat)
        {
            foreach (var survey in sortedSurveys)
            {
                ...
                {
                    var ListIdHDT = new int?[] { 1, 2 };
                    if (ListIdHDT.Contains(survey.id_hedaotao))
                    {
                        var ctdt = await db.ctdt.Where(x => x.id_hdt == giamsat.id_hdt).ToListAsync();
                    ...
                    {
                        var ctdt = await db.ctdt.ToListAsync();
                        foreach (var _ctdt in ctdt)
                        {
                            var mucdohailong = new List<dynamic>();
                            await muc_do_hai_long(survey, mucdohailong, _ctdt.id_ctdt, survey.surveyID, giamsat);
